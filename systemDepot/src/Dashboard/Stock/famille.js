...
function Familles() {
        ...
        if (!familleD.Name) {toast.error("Non de La famille est Invamlide !", GConf.TostErrorGonf)}
            ...
            axios.post(`${GConf.ApiLink}/stock/familles/ajouter`, {
                ...
                if(response.data.affectedRows) {
                    ...
                    SaveNotification('stockSaveFamille',GConf.SystemTag, familleD)
                }
                else{
    ...
    const EditFamille = () => {
        ...
        axios.post(`${GConf.ApiLink}/stock/familles/modifier`, {
            ...
            if(response.data.affectedRows) {
                ...
                setUpdateFT(Math.random() * 10)
                setLS(false)
                SaveNotification('stockEditFamille',GConf.SystemTag, editfamilleD)
            }
            else{
                setModalS(false)
                toast.error('Erreur esseyez de nouveaux', GConf.TostSuucessGonf)
   ...
   /*#########################[Card]##################################*/
    ...
    return ( <> 
            ...
            <div className="row">
                ...
                <Bounce bottom>
                    <div className="card card-body shadow-sm mb-4 sticky-top" style={{top:'70px' , zIndex:'1'}}>
                        ...
                        <Input icon='tags' onKeyPress={event => OnKeyPressFunc(event)} iconPosition='left' value={familleD.Name} placeholder='Non de la famille' className='w-100 border-0 rounded ' onBlur={checkFamilleExistance} onChange={(e) => setFamilleD({...familleD, Name: e.target.value })}/>
                        <h5 className='mb-1'>Description </h5>
                        <Form>
                            <TextArea onKeyPress={event => OnKeyPressFunc(event)} rows="3" placeholder='Designer la famille ' value={familleD.Description} className='w-100' onChange={(e) => setFamilleD({...familleD, Description: e.target.value })}/>
                        </Form>
                        <br />
            ...
            <EditModal setModalS={setModalS} EditFamille={EditFamille} editfamilleD={editfamilleD}  setEditFamilleD={setEditFamilleD} modalS={modalS} OnKeyPressFunc={OnKeyPressFunc} />
