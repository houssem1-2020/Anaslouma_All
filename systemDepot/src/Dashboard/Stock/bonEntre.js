...
function BonsEntre() {
          ...
          .then(function (response) {    
            if(response.data.affectedRows) {
                axios.post(`${GConf.ApiLink}/stock/bebs/save`, { 
                    tag: GConf.SystemTag, artList: bonsD, 
                }).then(function (response) { 
                    setBEID(response.data.BE_ID)  
                    SaveNotification('stockSaveBE',GConf.SystemTag, response.data.BE_ID) 
                })
    ...
    const MakeTotaleSum = () =>{
        let tot = 0
        bonsD.articles.map( (art) => { 
           const searchObject = articleList.find((article) => article.A_Code == art.A_Code);
           if (searchObject) {
                tot = tot +  (parseFloat(searchObject.Prix_vente) * art.NewQte)
           } else {
               tot = tot 
           }
                ...
                <div className='card card-body shadow-sm mb-2'>
                    ...
                    <div className='row mb-2'>
                        ...
                        <div className='col-12 mt-2'>
                            <Button  className='rounded-pill btn-imprimer'   disabled={!updateBtnState}  fluid onClick={(e) => PrintFunction('printBESR')}><Icon name='print' /> Imprimer En tant que bon de retour </Button>
                        </div>
                    </div>
                </div>
        </>)
    ...
    const AddArticles = () =>{
        return (<>
                <div className='card card-body shadow-sm mb-2'>
                    ...
                    <Input icon='pin' onKeyPress={event => OnKeyPressFunc(event)} list="articlesList"  onBlur={ (e) => GetArticleData(e.target.value)} size="small" iconPosition='left' placeholder='Desg'  fluid className='mb-1' /> 
    ...
    return ( <>
            ...
            <div className='row'>
                ...
                <div className='col-12 col-lg-7'>
                        <div className='card card-body shadow-sm mb-2 sticky-top rounded-pill' style={{top:'70px'}}>
                            <div className='row'>
                                <div className='col-7 align-self-center text-start'><h1>Totale : {MakeTotaleSum()}</h1></div>
                                <div className='col-5 align-self-center text-center'><h5 className='mb-0'>{bonsD.articles.length}</h5> articles</div>
                            </div>
                        </div>
            ...
            <FrameForPrint frameId='printBESR' src={`/Pr/Stock/BonR/${BE_ID}`} />
    </> );
}
