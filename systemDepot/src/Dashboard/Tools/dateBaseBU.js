...
function DataBaseBU() {
    ...
    /* ############################### UseEffect ################################*/
    useEffect(() => {
        console.log(new Date().toLocaleDateString('fr-FR').split( '/' ).join( '-' ))
    ...
    const FacturesCard = () =>{
        return(<>
            <div className='card card-body shadow-sm mb-4 border-div'>
               ...
               <div className='text-end'>
                    <Button className='bg-info rounded-pill' onClick={() => ExportFlies( `${GConf.SystemTag}_facture`,'Factures') }> <span className='bi bi-folder-symlink-fill'></span> Export</Button>
