...
const ButtonsCard = ({saveBtnState,EditFond,loaderState, editFondD, setEditFondD}) =>{
    return (<>
            <div className='card card-body shadow-sm mb-2'>
                <h5>Date & Confirmation</h5>
                <div className='mb-2'>
                    <Input icon='calendar alternate' type='date' size="small" iconPosition='left'   fluid className='mb-1' value={editFondD.jour} onChange={(e) => setEditFondD({...editFondD, jour: e.target.value })}/>               
                </div>
   ...
   /* ############################### [Const] ################################*/
    ...
    const panes = [
        ...
        },
        {
            ...
            render: () =><ButtonsCard saveBtnState={saveBtnState} EditFond={EditFond} loaderState={loaderState} editFondD={editFondD} setEditFondD={setEditFondD}  />,
        },
    ...
    const AddArticleToList = ()=>{
        ...
        if (!articleNow.A_Code) {toast.error("Code a Barre Invalide !", GConf.TostErrorGonf)}
        else if (!articleNow.Name ) {toast.error("Nom de l'article Invalide!", GConf.TostErrorGonf)}
        ...
        else if (!articleNow.Quantite && articleNow.Quantite != 0 ) {toast.error("Quantite Invalide ", GConf.TostErrorGonf)}
        else if (!articleNow.Qte && articleNow.Qte != 0) {toast.error("Ancien Quantite  Invalide!", GConf.TostErrorGonf)}
        else if (!articleNow.QteAjoute ) {toast.error("Qte a ajouter Invalide !", GConf.TostErrorGonf)}
        else{
            ...
            if (searchObject) {
                ...
                setArticleNow([])
                    ...
                    let arrayToAdd = {id:editFondD.articles.length + 1 ,A_Code: articleNow.A_Code, Name: articleNow.Name, Prix_vente: articleNow.Prix_vente, Groupage : articleNow.Groupage, Quantite: (articleNow.Quantite - articleNow.QteAjoute) , Qte: soitEnCamion , QteAjoute: articleNow.QteAjoute}
    ...
    const GetArticleData = (value) =>{
        ...
        if(!searchObject.Quantite) {searchObject.Quantite = 0}
            ...
            if (!editFondD.camion) {toast.error("Client est Invalide !", GConf.TostErrorGonf)}
                ...
                .then(function (response) {
                    if(response.status = 200) {
                        ...
                        SaveNotification('camionFondModifier',GConf.SystemTag, editFondD)
                        setLS(false)
                    }
                    else{
