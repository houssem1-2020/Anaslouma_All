...
const EditClientCard = ({clientD, setClientD, EditClient,delegList,GetDelegList,loaderState, OnKeyPressFunc }) =>{
    return(<>
            <div className='p-1'>
                        <div className='p-1 mb-2'>
                            <h5 className='mb-1'>Matricule Fiscale:</h5>
                            <Input icon='key' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} placeholder='Matricule Fiscale' className='w-100 border-0 shadow-sm rounded mb-1' disabled={true} value={clientD.Code_Fiscale}  onChange={(e) => setClientD({...clientD, Code_Fiscale: e.target.value })}/>
                        </div>
                        <div className='p-1  mb-2'>
...
function ClientInfo() {
    ...
    const panes = [
        ...
        },
        {
            ...
            render: () => <><Tab.Pane className='border-div' attached={false}><EditClientCard clientD={clientD} setClientD={setClientD} EditClient={EditClient} delegList={delegList} GetDelegList={GetDelegList}  loaderState={loaderState} OnKeyPressFunc={OnKeyPressFunc} /></Tab.Pane><br /></>,
        },
        {
    ...
    useEffect(() => {
        ...
        axios.post(`${GConf.ApiLink}/client/info`, {
            ...
            if(!response.data[0].PK) {
                ...
                //console.log(response.data)
                let toFacture = []
                response.data[0].Facture.map( (getData) => toFacture.push([ getData.F_ID, response.data[0].Name, new Date(getData.Cre_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ) , getData.Tota, 
                    _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/ft/info/${getData.F_ID}`)}><Icon  name='angle right' /></Button>)
                ],))
                setFactures(toFacture)

                let toCommande = []
                response.data[0].Commandes.map( (getData) => toCommande.push([ getData.C_ID, new Date(getData.Date_Passe).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ) , new Date(getData.Date_Volu).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ) , getData.Totale, getData.State,
                    _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/rq/info/${getData.C_ID}`)}><Icon  name='angle right' /></Button>)
                ],))
                setCommande(toCommande)

                let toFactureCam = []
                response.data[0].FactureCamion.map( (getData) => toFactureCam.push([ getData.F_ID, response.data[0].Name,  new Date(getData.Cre_Date).toLocaleDateString('fr-FR').split( '/' ).reverse( ).join( '-' ) , getData.Tota, 
                    _(<Button className='rounded-pill bg-system-btn' size='mini' onClick={ (e) => NavigateFunction(`/S/cm/info/facture/${getData.Camion}/${getData.F_ID}`)}><Icon  name='angle right' /></Button>)
                ],))
                setFactureCamion(toFactureCam)
        ...
        if (!clientD.Code_Fiscale) {toast.error("Matricule Invalide !", GConf.TostErrorGonf)}
            ...
            axios.post(`${GConf.ApiLink}/client/modifier`, {
                ...
                if(response.data.affectedRows) {
                    ...
                    SaveNotification('clientEdit',GConf.SystemTag, clientD)
                    setLS(false)
                }
                else{
