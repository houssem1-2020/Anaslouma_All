...
function StockPage() {
    ...
    /*#########################[Use Effect]##################################*/
    useEffect(() => {
        axios.post(`${GConf.ApiLink}/User/stock`, {
            tag: userData.tag,
          ...
          .then(function (response) {
            ...
            getData.Name,
             getData.Quantite,
             _(<h6><a href={`Stock/info/${getData.A_Code}`} ><Button className='rounded-pill bg-system-btn' size='mini'><Icon  name='angle right' /></Button></a></h6>)
            ],))
