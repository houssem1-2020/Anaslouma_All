...
import { Autocomplete, Backdrop, CircularProgress,     Slider, TextField, Typography } from '@mui/material';
...
const ActionCard = ({rating,setRating,saveFunction, rendyVousD, setRdvData, myPosition, setMyPosition, targetPosition, setTragetPosition}) => {
    ...
    const SelectPlaceInput = (valueT) => {
         
        setMyPosition([valueT.Lat,valueT.Lng])
        setTragetPosition(([valueT.Lat,valueT.Lng]))
    }
    ...
    return(<>
        <div className='card card-body bg-transparent rounded-2 border-0 mb-2 shadow-sm-s'>
            <h5>Sous Service  ? </h5> 
                {/* <div className="mt-1 p-1 mb-4 d-inline"   style={{width:'100%', overflowX: 'auto', overflowY : 'hidden', whiteSpace:'nowrap'}}> */}
                    ...
                    <Stack direction="row" className='mb-4' spacing={1}  sx={{ flexWrap: 'nowrap',  overflowX: 'auto',  }}>
                    {GConf.profSousTag['refection-charpentes'].map( (carouselData,index)=> 
                            <Chip
                                key={index}
                                label={carouselData.text}
                                clickable
            ...
            <div className='row'>
                        <div className='col-10'><h3 className='mb-2 mt-2 text-danger'>{rating[0]} - {rating[1]} Euro</h3></div>
                        {/* <div className='col-2 align-self-center'><Input type='number' size='mini' fluid value={rating} onChange={(e) => setRating(e.target.value)}/> </div>*/}
            ...
            <Slider
                ...
                //min={3}
                //max={500}
                getAriaValueText={() => {return `${rating}°C`}}
            ...
            <h5>Urgence  ? </h5> 
             <div className='row'>
                <div className='col-3'>
                    <div className={`card p-2 rounded-pill text-center border-2 ${targetIndex == 1 ? 'bg-success text-white':'text-success bg-transparent'}`} onClick={() => setTargetIndex(1)}>urgant</div>
                </div>
                <div className='col-4'>
                    <div className={`card p-2 rounded-pill text-center border-2 ${targetIndex == 2 ? 'bg-danger text-white':'text-danger bg-transparent'}`} onClick={() => setTargetIndex(2)}>Tres Urgent</div>
                </div>
                <div className='col-5'>
                    <div className={`card p-2 rounded-pill text-center border-2 ${targetIndex == 3 ? 'bg-warning text-white':'text-warning bg-transparent'}`} onClick={() => setTargetIndex(3)}>Asser urgent</div>
                </div>
             </div>
            ...
            <h5>Paymment  ? </h5> 
             <div className='row'>
                <div className='col-6'>
                    <div className={`card p-2 rounded-pill text-center border-2 ${activePIndex == 1 ? 'bg-secondary text-white':'text-secondary bg-transparent'}`} onClick={() => setActivePIndex(1)}>Sur Place </div>
                </div>
                <div className='col-6'>
                    <div className={`card p-2 rounded-pill text-center border-2 ${activePIndex == 2 ? 'bg-secondary text-white':'text-secondary bg-transparent'}`} onClick={() => setActivePIndex(2)}>En Ligne</div>
                </div>
             </div>
            
        </div>
        <div className='card bg-transparent card-body border-0 border-div-s mb-2 shadow-sm-s'>
            <h5>Position & Confirmation ? </h5> 
            ...
            <Autocomplete
                ...
                options={fetchedList}
                className='mb-3'
                renderInput={(params) => <TextField {...params} label="Votre position" onChange={(e) => FetchPlacesData(e.target.value)} />}
                loading={fetchingState}
                onChange={(event, data) => SelectPlaceInput(data)}
                // renderOption = {(option) => <CustomOption option={option} />}
            />

            <MapContainer  center={myPosition} zoom={15} scrollWheelZoom={false} className="map-height-small cursor-map-crosshair border-div mb-2"  >
                    <TileLayer
            ...
            <FormGroup>
                <FormControlLabel required className='p-3' color="success" control={<Checkbox />} label=" Accepter les conditions" />
            </FormGroup>
...
function ArticleInfo() {
    ...
    /* ############### Card #################*/
        const TopCard = () =>{
                return(<>
                        <div className='card card-body border-0 shadow-sm-s system-color-s rounded-0 fixed-top' style={{zIndex: 900, backgroundColor : '#f9fafb'}} >
                                <div className='row'>
    ...
    return ( <>
            ...
            <br />
            <div className='container'>
            <ActionCard rendyVousD={rendyVousD} setRdvData={setRdvData} rating={rating} setRating={setRating} saveFunction={saveFunction} myPosition={myPosition} setMyPosition={setMyPosition} targetPosition={targetPosition} setTragetPosition={setTragetPosition} />
            </div>
