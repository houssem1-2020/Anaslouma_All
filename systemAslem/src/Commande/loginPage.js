...
function InputLoginPage() {
    ...
    let Offline = JSON.parse(localStorage.getItem(`${GConf.SystemTag}_Cmd_Offline`));
     ...
     /*#########################[UseEffect]##################################*/
    useEffect(() => {
        const CommandeIsLogged = localStorage.getItem(`${GConf.SystemTag}_Cmd_Offline`);
        if (CommandeIsLogged) {window.location.href = "/C/L";}
            ...
            axios.post(`${GConf.ApiCommandeLink}/LogIn`, {
                ...
                if(response.data[0] == 'true') {
                    toast.success("Connectée !", GConf.TostSuucessGonf)
                        if (!Offline) {
                            localStorage.setItem(`${GConf.SystemTag}_Cmd_Offline`, JSON.stringify({stock: [], commandes: [],  client:[],  commandeToSave:[], pannierArticles:[], clientToSave:[]}));
                        }
                        //localStorage.setItem('InputCommande', response.data[1]);
                        localStorage.setItem(`${GConf.SystemTag}_Cmd_LocalD`, response.data[2]);
