...
function AjouterFacture() {
    ...
    const [factureD, setFactureD] = useState({client:'PASSAGER', de:'Sidi Bourouis', vers:'', Fournisseurs:'INDEFENIE', Chauffeur:'', jour: Today.toISOString().split('T')[0], totale: 0 , prixGros: false,  articles:[]})
    ...
    const SaveFacture = () =>{
           if (!CheckClientValidite(factureD.client)) {toast.error("Client n\'est Invalide !", GConf.TostErrorGonf)}
                ...
                .then(function (response) {
                    if(response.status = 200) {
                        ...
                        setLS(false)
                        setUSBS(false)
                        SaveNotification('factureAjouter',GConf.SystemTag, factureD)
                    }
                    else{
    ...
    const AddArticles = () =>{
        return (<>
                <div className='card card-body shadow-sm mb-2'>
                    ...
                    <Input icon='pin' list="articlesList" onKeyPress={event => OnKeyPressFunc(event)} placeholder='Entre aarticle'  onBlur={ (e) => GetArticleData(e.target.value)} size="small" iconPosition='left'   fluid className='mb-1' /> 
                    ...
                    <div className='row'>
                       <div className='col-8'>  <Input icon='dropbox' type='number'  autoFocus={autofocusState} onChange={ (e) => {articleNow.Qte = e.target.value}} size="small" iconPosition='left' placeholder={articleNow.Qte ? articleNow.Qte : 'Quantite'}   fluid className='mb-1' /> </div> 
