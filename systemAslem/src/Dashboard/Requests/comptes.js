...
const EditModal = ({setModalS,EditCompte,editCompteD,setCompteEdit,modalS, OnKeyPressFunc}) =>{
    return(<>
            <Modal
                    size='mini'
                    open={modalS}
                ...
                >
                    ...
                    <Modal.Content> 
                            <h5 className='mb-1 mt-2'>Nom & Prenom  </h5>
                                <Input icon='user plus' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} value={editCompteD.Name} placeholder='Nom & Prenom' className='w-100 border-0 rounded ' onChange={(e) => setCompteEdit({...editCompteD, Name: e.target.value })}/>  
                            <h5 className='mb-1 mt-2'>Identifiant  </h5>
                                <Input icon='user plus' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} value={editCompteD.Identifiant} placeholder='Identifiant' className='w-100 border-0 rounded ' onChange={(e) => setCompteEdit({...editCompteD, Identifiant: e.target.value })}/>  
                            <h5 className='mb-1 mt-2'>Mot De Passe </h5>
                                <Input icon='key' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} value={editCompteD.Password} placeholder='Mot De Passe' className='w-100 border-0 rounded ' onChange={(e) => setCompteEdit({...editCompteD, Password: e.target.value })}/>
                            <br />
                    </Modal.Content>
                    <Modal.Actions>
                ...
                <div className="row">
                    ...
                    <Bounce bottom>
                        <div className="card card-body shadow-sm mb-4 sticky-top" style={{top:'70px' , zIndex:'1'}}>
                            ...
                            <h5 className='mb-1 mt-2'>Nom & Prenom  </h5>
                                <Input icon='user plus' onKeyPress={event => OnKeyPressFunc(event)} iconPosition='left' value={compteData.Name} placeholder='Nom & Prenom' className='w-100 border-0 rounded ' onBlur={checkComptesExistance} onChange={(e) => setCompteData({...compteData, Name: e.target.value })}/>  
                            <h5 className='mb-1 mt-2'>Identifiant  </h5>
                                <Input icon='user plus' onKeyPress={event => OnKeyPressFunc(event)} iconPosition='left' value={compteData.Identifiant} placeholder='Identifiant' className='w-100 border-0 rounded ' onBlur={checkComptesExistance} onChange={(e) => setCompteData({...compteData, Identifiant: e.target.value })}/>  
                            <h5 className='mb-1 mt-2'>Mot De Passe </h5>
                                <Input icon='key' iconPosition='left' onKeyPress={event => OnKeyPressFunc(event)} value={compteData.Password} placeholder='Mot De Passe' className='w-100 border-0 rounded ' onBlur={checkComptesExistance} onChange={(e) => setCompteData({...compteData, Password: e.target.value })}/>
                            <br />
                            <div className='text-end'>
                                <Button  className={`text-end bg-system-btn rounded-pill`} disabled={saveBtnState} onClick={SaveCompte}>   <Icon name='save' /> Enregistrer <Loader inverted active={loaderState} inline size='tiny' className='ms-2'/></Button>
                            </div>
                        </div>
                    </Bounce>
                    </div>
                </div>
                <EditModal setModalS={setModalS} EditCompte={EditCompte} editCompteD={editCompteD}  setCompteEdit={setCompteEdit} modalS={modalS} OnKeyPressFunc={OnKeyPressFunc} />
