...
function StockPage() {
        ...
        .then(function (response) {
          if (!response.data) {
              ...
              response.data.map( (getData) => articleListContainer.push([
                _(<TableImage image={getData.Photo_Path} forStock fixed={getData.Stock_Fixe == 'true'} onClick={() => openEditModal(getData,true)}/>),
                getData.A_Code,
                getData.Name,
                getData.Genre,
                getData.Quantite,
    ...
    return (<>
              ...
              <Fade> 
                <TableGrid tableData={articleList} columns={GConf.TableHead.stock} page={articleList.length > 1 ? 'stock' : false} />
              </Fade>  
              <Modal
                      size='small'
                      open={modalS}
