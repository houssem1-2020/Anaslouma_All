   ...
   /*#########################[UseEffect]##################################*/
    ...
    const AddArticleToList = ()=>{
        if (!articleNow.A_Code) {toast.error("Code à barre Invalide !", GConf.TostErrorGonf)}
            ...
            if (searchObject) {
                ...
                let arrayToUpdate = {A_Code : articleNow.A_Code, Name: articleNow.Name, OldQte: articleNow.Quantite,  NewQte: parseInt(articleNow.Value), PureQte: parseInt(articleNow.Quantite - articleNow.Value)}
    ...
    const UpdateStock = () =>{
        ...
        axios.post(`${GConf.ApiLink}/stock/bs`, {
          ...
          .then(function (response) {    
            if(response.data.BE_ID) {
                toast.success("Article Modifier !", GConf.TostSuucessGonf)
                setBEID(response.data.BE_ID)  
                SaveNotification('stockSaveBE',GConf.PID, response.data.BE_ID) 
                setLS(false)
    ...
    const AddArticles = () =>{
        return (<>
                <div className='card card-body border-div mb-2'>
                    ...
                    <Input icon='asl' type='number' autoFocus={autofocusState} onKeyPress={(e) => AddByKeyPressed(e)} onChange={ (e) => {articleNow.Value = e.target.value}} size="small" iconPosition='left' placeholder='Valeur'  fluid className='mb-1' />
                    <br />
