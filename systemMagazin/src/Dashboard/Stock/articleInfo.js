...
const EditArticle = ({articleD, setArticleD, checkPrixCompatiblite, familles, EditArticleFunction,loaderState,setArticleRaccourci,loading,updateQte}) =>{
    return(<>
                ...
                <div>
                    <h5>Ajouter Raccourci ? </h5>
                    <div className="form-check form-switch">
                        {!loading ? ''
                        :
                        <input className="form-check-input form-check-input-lg" type="checkbox" defaultChecked={articleD.Fast_Input == 'true'}  onChange={() => setArticleRaccourci()}   />
                        }
...
function ArticleInfo() {
    ...
    const panes = [
        ...
        // },
        {
            ...
            render: () => <><Tab.Pane attached={false}><EditArticle loading={loading} articleD={articleD}  setArticleD={setArticleD} checkPrixCompatiblite={checkPrixCompatiblite} familles={familles} EditArticleFunction={EditArticleFunction} setArticleRaccourci={setArticleRaccourci} loaderState={loaderState} updateQte={updateQte} /></Tab.Pane><br /></>,
        },
        {
   ...
   /*#########################[UseEffect]##################################*/
    ...
    const setArticleRaccourci = (state) =>{
        
        axios.post(`${GConf.ApiLink}/stock/modifier/raccourci`, {
            tag :GConf.SystemTag,
            articleND :{A_Code : articleD.A_Code, Fast_Input : !JSON.parse(articleD.Fast_Input)},
        }).then(function (response) {
            console.log(response.data)
            if(response.data.affectedRows) {
                toast.success("Article Modifier !", GConf.TostSuucessGonf)
                setArticleD({...articleD, Fast_Input: JSON.stringify(!JSON.parse(articleD.Fast_Input)) })
